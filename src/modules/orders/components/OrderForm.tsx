...
export const OrderForm = ({ onSubmitSuccess, onCancel }: OrderFormProps) => {
  const { register, control, handleSubmit, formState: { errors } } = useForm<OrderFormValues>({
    ...
    defaultValues: {
      ...
      items: [{ productName: '', variations: [{ size: '', color: '', quantityOrdered: 1, quantityDelivered: 0 }] }]
    }
  });
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      <header className="bg-slate-900 px-8 py-6 text-white flex justify-between items-center sticky top-0 z-20">
        ...
        <button onClick={onCancel} className="hover:text-rose-500 transition-colors" aria-label="Cerrar">✕</button>
      </header>
      ...
      <form onSubmit={handleSubmit(onSubmitSuccess)} className="p-8 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {/* ... otros campos generales con id/htmlFor ... */}
        </div>
        ...
        <button 
          type="button"
          onClick={() => append({ productName: '', variations: [{ size: '', color: '', quantityOrdered: 1, quantityDelivered: 0 }] })}
...
// Sub-componente memoizado para evitar bottlenecks de renderizado
const VariationFields = memo(({ nestIndex, control, register }: { nestIndex: number, control: Control<OrderFormValues>, register: UseFormRegister<OrderFormValues> }) => {
  ...
  return (
    <div className="space-y-2 border-t border-slate-200 pt-4">
      ...
      <button type="button" onClick={() => append({ size: '', color: '', quantityOrdered: 1, quantityDelivered: 0 })} className="text-[9px] font-black text-blue-500 uppercase mt-2">
        + Añadir Talle/Color
      </button>
    </div>
  );
