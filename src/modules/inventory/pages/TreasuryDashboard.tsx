...
import { Wallet, Pencil, Trash2, Plus, X } from "lucide-react";
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      const result = await Swal.fire({
        ...
        text: "Esto alterará los saldos de caja y es irreversible.",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#e11d48",
        confirmButtonText: "SÍ, BORRAR",
        customClass: {
          popup: "dark:!bg-slate-900 !rounded-[2.5rem] !text-white",
        },
      });
      ...
      <header className="flex flex-col md:flex-row justify-between items-end gap-4">
        ...
        {!isFormOpen && (
          <button
            ...
            className="bg-slate-900 dark:bg-blue-600 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-[0.2em] shadow-xl active:scale-95 transition-all flex items-center gap-2"
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
              {transactions.map((tx) => {
                ...
                const pMethod =
                  tx.payment_method || tx.paymentMethod || "EFECTIVO";
          ...
          {transactions.length === 0 && !isLoading && (
            <div className="py-24 text-center">
              <Wallet className="w-16 h-16 mx-auto mb-4 text-slate-200 dark:text-slate-800" />
