...
export const ServicesDashboard = memo(() => {
  ...
  // 🚀 OPTIMIZACIÓN: Filtro memorizado y limpieza de espacios
  const filteredServices = useMemo(() => {
    const search = searchTerm.toLowerCase().trim();
    return services.filter(s => s.name.toLowerCase().includes(search));
  }, [services, searchTerm]);

  // 🛠️ ACCIÓN: Editar Servicio Blindado
  const handleEdit = useCallback(async (service: Service) => {
    const { value: formValues } = await Swal.fire({
      ...
      preConfirm: () => {
        ...
        if (!name || !price) return Swal.showValidationMessage('Completá nombre y precio');
        return { name: name.toUpperCase(), price: Number(price), description: (document.getElementById('sw-desc') as HTMLTextAreaElement).value.trim() };
      }
    });
  ...
  // 🛠️ ACCIÓN: Eliminar Blindado
  const handleDelete = useCallback(async (id: string) => {
    const result = await Swal.fire({
      title: '¿ELIMINAR TARIFA?',
      text: "Esta acción es irreversible y afectará los catálogos.",
      icon: 'warning',
      showCancelButton: true,
  ...
  const handleCreateNew = useCallback(async () => {
    const { value: formValues } = await Swal.fire({
      ...
      html: `
        <div class="text-left space-y-4 p-2">
          <div>
            <label class="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Nombre</label>
          ...
          <div>
            <label class="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-1">Descripción</label>
            <textarea id="new-desc" class="swal2-textarea !w-full !m-0 !mt-1 !bg-slate-50 dark:!bg-slate-900 !border-slate-200 dark:!border-slate-700 !text-sm !rounded-xl" rows="3" placeholder="Detalles operativos..."></textarea>
          </div>
        </div>
      `,
      ...
      preConfirm: () => {
        ...
        if (!name || !price) return Swal.showValidationMessage('Faltan datos obligatorios');
        return { name: name.toUpperCase(), price: Number(price), description: (document.getElementById('new-desc') as HTMLTextAreaElement).value.trim() };
      }
    });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredServices.length === 0 ? (
           <div className="col-span-full py-24 text-center opacity-30">
             <span className="text-7xl mb-4 block italic">⚙️</span>
            ...
            <div key={service.id} className="bg-white dark:bg-slate-800 p-8 rounded-[3rem] border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-xl hover:border-blue-500/30 transition-all group flex flex-col h-full relative overflow-hidden">
              <div className="flex justify-between items-start mb-6">
                 ...
                 <div className="flex gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(service)} className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-50 dark:bg-slate-700 text-slate-400 hover:text-blue-500 transition-all" aria-label="Editar">✏️</button>
                    <button onClick={() => handleDelete(service.id)} className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-50 dark:bg-slate-700 text-slate-400 hover:text-rose-500 transition-all" aria-label="Eliminar">🗑️</button>
                 </div>
              </div>
              ...
              {/* Decoración Holding */}
              <div className="absolute -right-4 -bottom-4 text-6xl opacity-[0.02] font-black italic select-none group-hover:scale-110 transition-transform">
                RAÍCES
              </div>
            </div>
          ))
