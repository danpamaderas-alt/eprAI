...
export const FinancialDashboard = memo(() => {
  ...
  const handleAddExpense = useCallback(async () => {
    const { value: formValues } = await Swal.fire({
      ...
      customClass: {
        ...
        confirmButton:
          "bg-rose-600 hover:bg-rose-500 text-white font-black px-6 py-4 rounded-2xl uppercase text-xs tracking-[0.2em] w-full mb-3 shadow-lg active:scale-95 transition-all",
        cancelButton:
          "bg-slate-100 dark:bg-slate-800 text-slate-500 font-black px-6 py-4 rounded-2xl uppercase text-xs tracking-widest w-full",
      },
      preConfirm: () => {
        const amount = Number.parseFloat(
          (document.getElementById("ex-amount") as HTMLInputElement).value,
        );
        const description = (
          document.getElementById("ex-desc") as HTMLInputElement
        ).value.trim();
        const category = (
          document.getElementById("ex-cat") as HTMLSelectElement
        ).value;
  ...
  if (isLoading)
    return (
      <div className="p-8 h-screen flex items-center justify-center font-black text-slate-500 uppercase animate-pulse tracking-[0.5em] italic">
        Analizando Patrimonio Holding...
      </div>
    );
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-10 animate-in fade-in duration-700">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-[2.5rem] shadow-sm">
          ...
          <p className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter tabular-nums">
            {ARS.format(treasuryMetrics.income)}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 rounded-[2.5rem] shadow-sm">
          ...
          <p className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter tabular-nums">
            {ARS.format(treasuryMetrics.expenses)}
          </p>
        </div>
        ...
        <div className="bg-slate-900 border border-blue-600 p-8 rounded-[2.5rem] shadow-xl shadow-blue-600/10 relative overflow-hidden group">
          ...
          <p className="text-3xl font-black text-white tracking-tighter tabular-nums">
            {ARS.format(moneyInStreet)}
          </p>
          <div className="absolute right-0 bottom-0 opacity-10 text-6xl p-4 group-hover:scale-110 transition-transform">
            🤝
          </div>
        </div>

        <div
          className={`p-8 rounded-[2.5rem] border-2 shadow-xl ${treasuryMetrics.net >= 0 ? "bg-emerald-500/5 border-emerald-500/20" : "bg-rose-500/5 border-rose-500/20"}`}
        >
          <p className="text-[10px] font-black uppercase mb-2 text-slate-500 tracking-widest italic">
            Saldo Neto (Caja Real)
          </p>
          <p
            className={`text-4xl font-black tracking-tighter tabular-nums ${treasuryMetrics.net >= 0 ? "text-emerald-500" : "text-rose-500"}`}
          >
      ...
      <section className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[3rem] p-10 shadow-sm">
        <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mb-10 italic">
          📦 Valuación de Mercadería Física
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          <div>
            <p className="text-[10px] font-black text-slate-500 uppercase mb-2">
              Inversión Activa (Costo)
            </p>
            <p className="text-2xl font-black text-slate-900 dark:text-white tabular-nums">
              {ARS.format(stockMetrics.stockCost)}
            </p>
          </div>
          <div>
            ...
            <p className="text-2xl font-black text-blue-600 dark:text-blue-400 tabular-nums">
              {ARS.format(stockMetrics.stockValue)}
            </p>
          </div>
          <div className="md:border-l md:border-slate-100 dark:md:border-slate-800 md:pl-10">
            ...
            <p className="text-4xl font-black text-emerald-600 dark:text-emerald-400 tracking-tighter tabular-nums">
              {ARS.format(stockMetrics.projectedProfit)}
            </p>
          </div>
          <div className="flex flex-col justify-center">
            <p className="text-[10px] font-black text-indigo-500 uppercase mb-2 tracking-widest">
              Margen de Catálogo
            </p>
            <p className="text-4xl font-black text-indigo-600 dark:text-indigo-400 tracking-tighter tabular-nums">
              {stockMetrics.avgMargin}%
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-slate-900 rounded-[3.5rem] p-12 relative overflow-hidden shadow-2xl border border-slate-800">
        <div className="absolute top-0 right-0 p-12 text-9xl opacity-5 font-black uppercase italic">
          RAÍCES
        </div>
        <div className="relative z-10">
          <p className="text-blue-500 text-[10px] font-black uppercase tracking-[0.5em] mb-4">
            Patrimonio Total Estimado (Caja + Calle + Inversión)
          </p>
          ...
          <div className="flex gap-4 mt-6">
            <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
              <span className="w-2 h-2 bg-blue-500 rounded-full"></span> Basado
