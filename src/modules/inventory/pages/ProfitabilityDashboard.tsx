      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 print:hidden">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setDateRange(dateRange === 'all' ? 'thisMonth' : dateRange === 'thisMonth' ? 'quarter' : dateRange === 'quarter' ? 'year' : 'all')}
            className="flex items-center gap-1.5 px-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-[10px] font-black uppercase text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors transition-transform active:scale-95">
                ...
                return (
                  <div key={cat.name}>
                    ...
                    <div className="h-5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden flex">
                      <div className="h-full bg-indigo-500 rounded-l-full transition-colors" style={{ width: `${width}%` }} />
                    </div>
                  </div>
                );
              })}
