...
// 🚀 OPTIMIZACIÓN: Tarjeta de producto memorizada
const ProductCard = memo(({ product, onAdd }: { product: Product, onAdd: (p: Product) => void }) => (
  ...
  >
    ...
    <p className="mt-4 text-2xl font-black text-blue-600 dark:text-blue-400 tabular-nums">{ARS.format(product.price || 0)}</p>
    <div className="absolute -right-2 -bottom-2 opacity-5 text-4xl group-hover:scale-110 transition-transform">📦</div>
  </button>
));

export const SalesDashboard = () => {
  const { products, inventory, fetchAllCatalogs, updateStock } = useCatalogStore();
  ...
  const addToCart = useCallback(async (product: Product) => {
    const variants = inventory.filter(v => v.product_id === product.id && v.stock_quantity > 0);

    if (!variants.length) {
      return Swal.fire({ title: 'Sin Stock', text: 'No hay unidades físicas disponibles.', icon: 'warning', background: '#0f172a', color: '#fff' });
    }
    ...
    const htmlOptions = variants.map(v => `
      <button type="button" class="swal-v-btn p-4 rounded-2xl border-2 border-slate-700 bg-slate-800 text-left hover:border-blue-500 transition-all flex flex-col" 
        data-id="${v.id}" data-max="${v.stock_quantity}" data-sid="${v.size_id}" data-cid="${v.color_id}" 
        ...
        <span class="text-[10px] font-bold text-emerald-400 mt-1">DISPONIBLE: ${v.stock_quantity}</span>
      </button>
    `).join('');
    ...
    const { value: res } = await Swal.fire({
      ...
      html: `
        <div class="text-left space-y-6 p-2">
          ...
          <input type="hidden" id="sw-sid"> <input type="hidden" id="sw-sname">
      ...
      preConfirm: () => {
        const qty = Number((document.getElementById('sw-qty') as HTMLInputElement).value);
        if (! (document.getElementById('sw-sid') as HTMLInputElement).value) return Swal.showValidationMessage('Elegí un talle/color');
        if (qty <= 0) return Swal.showValidationMessage('Cantidad inválida');
        return { 
    ...
    if (res) {
      setCart(prev => [...prev, {
        id: crypto.randomUUID(), product_id: product.id, name: product.name, price: product.price || 0,
        quantity: res.qty, size_id: res.sid, size_name: res.sname, color_id: res.cid, color_name: res.cname
      }]);
    }
  }, [inventory]);
    ...
    if (confirm.isConfirmed) {
      ...
      try {
        ...
        await supabase.from('sales').insert([{
          customer_id: selectedCustomerId, total_amount: totals.total, payment_method: paymentMethod, items: cart,
          status: paymentMethod === 'CTA_CTE' ? 'DEUDA' : 'COBRADO' 
        }]);

        // 3. FLUJO FINANCIERO (CTA CTE o TESORERÍA)
        if (paymentMethod === 'CTA_CTE') {
          await supabase.from('account_movements').insert([{
            customer_id: selectedCustomerId, amount: totals.total, movement_type: 'CARGO',
            description: `Venta POS: ${cart.length} prendas`
          }]);
          const currentBal = Number((cliente as any)?.balance || 0);
          await supabase.from('customers').update({ balance: currentBal + totals.total }).eq('id', selectedCustomerId);
        } else {
          await addTransaction({
            date: new Date().toISOString(), description: `VENTA POS: ${cliente?.name || 'CONSUMIDOR FINAL'}`,
            category: 'VENTA_CATALOGO', type: 'INCOME', business_unit: 'RAICES', 
            payment_method: paymentMethod as any, amount: totals.total, status: 'COMPLETED'
          });
        }
        ...
        Swal.fire({ title: 'FALLO CRÍTICO', text: err.message, icon: 'error' });
      } finally {
  ...
  return (
    <div className="flex h-screen gap-0 overflow-hidden bg-slate-50 dark:bg-slate-950 p-0">
      ...
      <div className="flex flex-1 flex-col space-y-4 overflow-hidden p-6">
        <header className="flex justify-between items-center mb-2">
           ...
           <div className="relative w-72">
              ...
              <input type="text" placeholder="Buscar artículo..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
      ...
      {/* SECCIÓN DERECHA: CARRITO Y CIERRE */}
          ...
          {selectedCustomerId ? (
            <div className="flex items-center justify-between bg-blue-600 p-5 rounded-[2rem] text-white shadow-lg animate-in zoom-in-95">
              <div className="overflow-hidden">
                ...
                <p className="text-sm font-black uppercase truncate">{customers.find(c => c.id === selectedCustomerId)?.name}</p>
              </div>
              <button onClick={() => { setSelectedCustomerId(null); setClientSearch(''); }} className="p-2 hover:bg-white/20 rounded-full transition-colors"><X className="w-5 h-5" /></button>
            </div>
          ) : (
            <div className="relative">
              <input type="text" placeholder="Buscar por nombre..." value={clientSearch} onChange={e => setClientSearch(e.target.value)}
              ...
              {filteredCustomers.length > 0 && (
                <div className="absolute z-50 w-full mt-2 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-[2rem] shadow-2xl overflow-hidden">
                  {filteredCustomers.map(c => (
                    <button key={c.id} onClick={() => { setSelectedCustomerId(c.id); setClientSearch(c.name); }}
                      className="w-full px-6 py-4 text-left text-xs font-black hover:bg-blue-50 dark:hover:bg-blue-900/40 dark:text-white border-b border-slate-50 dark:border-slate-800 last:border-none uppercase">{c.name}</button>
                  ))}
                </div>
              )}
            </div>
          ...
          {cart.map((item) => (
            <div key={item.id} className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-[2rem] border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-all group">
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => setCart(prev => prev.filter(i => i.id !== item.id))} className="p-2 text-slate-300 hover:text-rose-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
              </div>
              <div className="flex justify-between items-end mt-4">
                <p className="text-[10px] font-bold text-slate-400 uppercase">{item.quantity} x {ARS.format(item.price)}</p>
                <p className="font-black text-sm text-slate-900 dark:text-white tabular-nums">{ARS.format(item.price * item.quantity)}</p>
              </div>
            </div>
          ))}
          {cart.length === 0 && (
