...
import { useSublimationJobStore, type DeliverJobData } from '../store/useSublimationJobStore';
...
export const DeliverSublimationJobModal = ({ job, onClose }: Props) => {
  const toast = useToastStore((s) => s.toast);
  const deliverJob = useSublimationJobStore((s) => s.deliverJob);
  ...
  const specs: string[] = [];
  if (job.blank_label) specs.push(`Producto: ${job.blank_label}`);
  if (job.size_label) specs.push(`Tamaño: ${job.size_label}`);
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <h2 className="flex items-center gap-2 text-sm font-black uppercase tracking-widest text-white">
            <Truck size={16} className="text-fuchsia-400" aria-hidden />
            Entregar trabajo
          </h2>
          {!done && (
        ...
        {done ? (
          <div className="p-6 text-center space-y-4">
            ...
            <p className="text-sm font-black text-white">Trabajo entregado</p>
            <p className="text-xs text-slate-400 font-bold">
              Venta registrada · Remito <span className="text-fuchsia-300">{done}</span> generado con el detalle de producción.
            </p>
          ...
          <>
            <div className="px-6 pt-5 space-y-4">
              ...
              <p className="text-[10px] font-bold text-slate-600 leading-relaxed">
                Al entregar se registra la venta en Finanzas y se genera un remito DELIVERED con el detalle de la pieza.
              </p>
            </div>
            ...
            <div className="flex gap-3 px-6 py-4 mt-2 border-t border-slate-800">
              ...
              <button disabled={!canSave} onClick={() => void handleDeliver()}
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-fuchsia-600 hover:bg-fuchsia-500 disabled:opacity-40 disabled:hover:bg-fuchsia-600 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-colors">
