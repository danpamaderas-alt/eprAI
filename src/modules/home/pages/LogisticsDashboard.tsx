...
export const LogisticsDashboard = memo(() => {
  ...
  // 🚀 OPTIMIZACIÓN: Zonas únicas memorizadas
  const zones = useMemo(() => {
    const uniqueZones = new Set(deliveries.map(d => d.zone));
    return ['TODAS', ...Array.from(uniqueZones)];
  }, [deliveries]);

  // 🚀 OPTIMIZACIÓN: Filtrado y ordenamiento de prioridad logística
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Swal.fire('Error', 'No se pudo guardar el envío', 'error'); 
    }
  };

  // 🚀 FIX: URL de Maps corregida para compatibilidad universal
      ...
      {isFormOpen && (
        <form onSubmit={handleSave} className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-700 space-y-5 animate-in slide-in-from-top duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Cliente</label>
              <input required value={formData.customerName} onChange={e => setFormData({...formData, customerName: e.target.value})} className="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-bold outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" placeholder="Nombre para el repartidor" />
            </div>
            <div>
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">WhatsApp</label>
              <input value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-bold outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" placeholder="Sin el 15 (Ej: 2215554433)" />
            </div>
            <div className="md:col-span-2">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Dirección Exacta</label>
              <input required value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} className="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-bold outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" placeholder="Calle, N°, Timbre/Depto" />
            </div>
            <div>
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Zona de Reparto</label>
              <select value={formData.zone} onChange={e => setFormData({...formData, zone: e.target.value})} className="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-black uppercase text-xs outline-none focus:ring-2 focus:ring-blue-500 dark:text-white">
            ...
            <div className="md:col-span-2">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Descripción del Paquete</label>
              <input required value={formData.itemsDesc} onChange={e => setFormData({...formData, itemsDesc: e.target.value})} className="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-bold outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" placeholder="Ej: 2x Remeras Negras L, 1x Buzo XL" />
            </div>
            <div className="md:col-span-2">
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Notas para el Repartidor</label>
              <input value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none font-bold outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" placeholder="Ej: Dejar en el kiosco si no hay nadie" />
            </div>
          </div>
        ...
        {filteredDeliveries.map(delivery => (
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <span className={`text-[9px] font-black px-3 py-1 rounded-lg uppercase tracking-widest border ${statusColors[delivery.status as keyof typeof statusColors]}`}>
            ...
            <div className="border-t border-slate-100 dark:border-slate-700 pt-4 flex gap-2">
              {delivery.status === 'PENDING' && (
                <button onClick={() => updateDeliveryStatus(delivery.id, 'IN_TRANSIT')} className="flex-1 bg-blue-600 text-white py-3 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-blue-500/20 active:scale-95 transition-all">Iniciar Reparto</button>
              )}
              {delivery.status === 'IN_TRANSIT' && (
                <>
                  <button onClick={() => updateDeliveryStatus(delivery.id, 'DELIVERED')} className="flex-1 bg-emerald-500 text-white py-3 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-emerald-500/20 active:scale-95 transition-all">Entregado</button>
                  <button onClick={() => updateDeliveryStatus(delivery.id, 'FAILED')} className="flex-1 bg-rose-500 text-white py-3 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-rose-500/20 active:scale-95 transition-all">Falló</button>
                </>
              )}
              {(delivery.status === 'DELIVERED' || delivery.status === 'FAILED') && (
                <button onClick={() => deleteDelivery(delivery.id)} className="w-full bg-slate-50 dark:bg-slate-900 text-slate-400 dark:text-slate-600 hover:text-rose-500 py-3 rounded-xl text-[10px] font-black uppercase transition-colors">Archivar Envío</button>
              )}
            </div>
          </div>
        ))}
