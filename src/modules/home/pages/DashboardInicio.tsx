...
export const DashboardInicio = () => {
  // Datos simulados para la demostración visual (luego los conectaremos a la base real)
  const stats = {
    ventasMes: 1250000,
    crecimiento: '+15%',
    stockTotal: 1240,
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <header className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-blue-100 font-medium text-lg">Aquí tienes el resumen de Raíces al día de hoy.</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col justify-center relative overflow-hidden group">
          ...
          <div className="flex items-end gap-2">
            <p className="text-3xl font-black text-slate-900 dark:text-white">{formatCurrency(stats.ventasMes)}</p>
            <span className="text-xs font-bold text-emerald-500 mb-1">{stats.crecimiento}</span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col justify-center relative overflow-hidden group">
          ...
          <p className="text-3xl font-black text-slate-900 dark:text-white">{stats.stockTotal} <span className="text-sm text-slate-400 font-bold">unidades</span></p>
        </div>

        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col justify-center relative overflow-hidden group">
          <div className="absolute -right-4 -top-4 text-6xl opacity-5 group-hover:scale-110 transition-transform">🤝</div>
          <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-1">Clientes en CRM</p>
          <p className="text-3xl font-black text-slate-900 dark:text-white">{stats.clientes} <span className="text-sm text-slate-400 font-bold">activos</span></p>
        </div>
        ...
        <div className="bg-slate-900 dark:bg-slate-950 p-6 rounded-3xl border border-slate-800 shadow-lg flex flex-col justify-center relative overflow-hidden group">
          ...
          <p className="text-3xl font-black text-white">{stats.pedidosPendientes} <span className="text-sm text-slate-400 font-bold">pendientes</span></p>
        </div>
          ...
          <div className="space-y-3 flex-1">
            ...
            <Link to="/inventario" className="flex items-center gap-4 p-4 rounded-2xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/50 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors group">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-blue-600/70 dark:text-blue-500">Cargar mercadería</p>
              </div>
            </Link>
