...
export const useCatalogStore = create<CatalogState>((set, get) => ({
  sizes: [], colors: [], paymentMethods: [], businessUnits: [], products: [], customers: [], personalizationTypes: [], inventory: [], isLoading: false,

  fetchAllCatalogs: async () => {
    set({ isLoading: true });
    try {
      const [ resSizes, resColors, resPayments, resUnits, resProducts, resCustomers, resPerso, resInventory ] = await Promise.all([
      ...
      set({ 
        sizes: resSizes.data || [], colors: resColors.data || [], paymentMethods: resPayments.data || [],
        businessUnits: resUnits.data || [], products: resProducts.data || [], customers: resCustomers.data || [],
        personalizationTypes: resPerso.data || [], inventory: resInventory.data || [], isLoading: false 
      });
    } catch (error) { 
  ...
  updateProductComplete: async (productId, updates) => {
    ...
    } catch (error) { console.error('Error al actualizar producto:', error); throw error; }
  },
  ...
  updateStock: async (productId, sizeId, colorId, quantity) => {
    try {
      ...
      await get().fetchAllCatalogs();
    } catch (error) { console.error('Error al actualizar stock:', error); throw error; }
  },

  // 🔥 RESTAURAMOS LAS FUNCIONES FALTANTES
  ...
  registerPayment: async (customerId, amount, notes) => {
    try {
      ...
      await get().fetchAllCatalogs();
    } catch (error: any) { console.error('Error Crítico:', error.message); throw error; }
  }
}));